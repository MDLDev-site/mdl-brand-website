...
## MDL Security Patterns

### Cognito Multi-Tenant Auth

MDL uses AWS Cognito with **one user pool per channel** (sports/media organisation). This is the multi-tenancy boundary. The custom Lambda authorizer (`apiAuthorizer` for fan-facing, `authorizerAdminApp` for admin) validates the JWT and injects tenant context:

```javascript
// Always validate authorizer context is present
export const handler = async (event) => {
  const auth = event.requestContext?.authorizer?.lambda;
  if (!auth?.channel_id || !auth?.user_id) {
    return { statusCode: 401, body: JSON.stringify({ error: 'Unauthorized' }) };
  }
  const { channel_id, user_id } = auth;
  // ...
};
```

Never trust `channel_id` from the request body or query string — only from the authorizer context.

### Lambda Handler Security Checklist

Before shipping any new Lambda handler:
- [ ] Authorizer context validated (`channel_id` + `user_id` present)
- [ ] Request body validated with Zod schema before use
- [ ] All DynamoDB queries scoped to `channel_id`
- [ ] No secrets hardcoded — use `process.env` backed by AWS Secrets Manager
- [ ] CORS handled by the appropriate CORS Lambda (not inline)
- [ ] Error responses don't leak internal details (stack traces, table names)

### Stripe Webhook Security

All Stripe webhooks **must** go through `stripeWebhookVerify` before processing. Never process a webhook payload without signature verification:
...
export const handler = async (event) => {
  ...
  // now safe to process stripeEvent
};
```

### DRM and Media Security

- DRM profiles managed via `apiDrm` Lambda and `drm_profile_crud`
- DRM keys must never be logged or returned in API responses
- Bitmovin DRM config is per-channel — always scope by `channel_id`
- Signed URLs for protected media via CloudFront

### Frontend Security (Fan App)

- **Tokens:** Use `amazon-cognito-identity-js` session management — never manually store JWTs in `localStorage`
- **Sentry:** Only log user IDs (not email, name, or payment data). Configure Sentry `beforeSend` to scrub PII.
- **reCAPTCHA v3:** Verify token server-side on signup Lambda (`authSignup`), not just client-side
- **Stripe:** Use `@stripe/react-stripe-js` — never handle raw card data; always use Stripe Elements
...
Cache secrets within the Lambda execution context (module-level variable) — don't fetch on every invocation.

### CORS Pattern

CORS is handled by dedicated Lambda functions, not inline. When adding a new API route:
1. Add the route to the appropriate CORS Lambda (`apiCorsHandler` for admin/fan, `cbeCORSHandler` for CBE)
2. Never add `Access-Control-Allow-Origin: *` in individual Lambdas

### Common MDL Security Mistakes

- ❌ Trusting `channel_id` from request body instead of authorizer context
- ❌ Processing Stripe webhooks without signature verification
- ❌ Logging email addresses or payment data to CloudWatch/Sentry
- ❌ Returning raw DynamoDB errors to clients (leaks table/key structure)
- ❌ Hardcoding API keys or secrets in Lambda code
- ❌ Querying DynamoDB without `channel_id` scope
