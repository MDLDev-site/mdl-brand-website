...
## MDL Performance Patterns

### Lambda Cold Starts

Cold start adds 300–800ms for Node.js Lambdas. Mitigations:

```javascript
// ✅ Module-level init — reused on warm invocations
const docClient = DynamoDBDocumentClient.from(new DynamoDBClient({}));
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);

// ❌ Inside handler — re-initialised on every invocation
export const handler = async (event) => {
  const docClient = DynamoDBDocumentClient.from(new DynamoDBClient({})); // bad
};
```

- Use provisioned concurrency for the critical path: auth (`apiAuthorizer`), FFE media items, stream dashboard
- Keep Lambda package size small — don't `npm install` packages the function doesn't use
- Each function has its own `package.json` — only install what that function needs
- Avoid importing the entire `shared/` module if only one utility is needed

### DynamoDB Query Optimisation

```javascript
// ✅ Query with projection — fetch only needed fields
const { Items } = await docClient.send(new QueryCommand({
  TableName: 'media_items',
  KeyConditionExpression: 'channel_id = :cid',
  ExpressionAttributeValues: { ':cid': channelId },
  ProjectionExpression: 'media_id, title, thumbnail_url, duration', // not '*'
}));

// ❌ Scan — never in hot paths
await docClient.send(new ScanCommand({ TableName: 'media_items' })); // kills perf + costs
```

Watch DynamoDB `ConsumedReadCapacityUnits` in CloudWatch. If throttles appear, add a GSI or increase capacity.

### Bitmovin Player — Video Start Time

Video start time is the most user-visible performance metric for MDL fans.

- Preload the Bitmovin player SDK early (don't lazy-load on play button click)
- Configure ABR start at a low bitrate, then ramp up — reduces initial buffering
- Use CloudFront for HLS segment delivery — keep CDN cache hit rate > 95%
- Monitor VST (Video Start Time) in Bitmovin Analytics — target < 3s
- Pre-warm the HLS manifest request: start fetching manifest when the page loads, not on play

### Frontend Bundle (fan-dev — Vite)

```bash
# Analyse bundle size
npx vite-bundle-analyzer  # or check rollup output
```

Fan-dev uses `NODE_OPTIONS=--max-old-space-size=8192` at build — the bundle is large. Optimisations:

- Route-based code splitting is already configured with `React.lazy` — keep large libraries in async chunks
- Bitmovin Player is heavy — ensure it's in a separate chunk, only loaded on video pages
- TanStack Query DevTools: only import in dev mode (`import.meta.env.DEV`)
- Embla Carousel and Swiper: don't import both — pick one per use case

### TanStack Query Cache Tuning

Avoid redundant API calls with appropriate cache config:

```typescript
// Fan content (changes infrequently)
useQuery(['media-items'], fetchMediaItems, {
  staleTime: 5 * 60 * 1000,  // 5 min — don't refetch if fresh
  cacheTime: 10 * 60 * 1000, // 10 min — keep in memory
});

// Live stream status (changes frequently)
useQuery(['stream-status', streamId], fetchStreamStatus, {
  staleTime: 10_000,          // 10s — refresh often
  refetchInterval: 15_000,    // auto-poll every 15s when live
});
```

### Critical Path: Fan Video Playback

The sequence that most affects perceived performance:

```
1. Page load → Cognito token refresh (< 200ms if cached)
2. Fetch media item details → DynamoDB query (< 100ms with proper key)
3. Fetch DRM licence URL → apiDrm Lambda (< 300ms)
4. Bitmovin loads HLS manifest → CloudFront (< 200ms)
5. First HLS segment → CloudFront (< 500ms)
─────────────────────────────────────────────
Target: video playing within 3s of page load
```

Monitor each step independently. Bitmovin Analytics captures steps 4–5 automatically.

### CloudFront Cache Strategy

- HLS segments: long TTL (1 hour+) — content doesn't change
- HLS manifests: short TTL (10–30s for live, 5 min for VOD)
- Thumbnails: long TTL (24 hours) — add cache-busting URL params on update
- Lambda API responses: no cache at CloudFront level — handled by TanStack Query on frontend
