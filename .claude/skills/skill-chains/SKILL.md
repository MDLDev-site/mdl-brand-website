...
## MDL Astro Site Workflows

### New Astro Brand Site — Full Build

**Use for:** Greenfield Astro site (MDL brand site and similar)

**Chain:** Astro Specialist → Security Sentinel → Performance Engineer → Localization & i18n Engineer → DevOps/IaC Specialist → Technical Writer

**Workflow:**

1. **Astro Specialist** — Project scaffold, rendering mode decision (static/hybrid), integration selection (Tailwind, MDX, sitemap), content collection schemas, component architecture
   - *Output:* Configured `astro.config.mjs`, typed content collections, layout system
2. **Security Sentinel** — CSP headers, env var strategy, API route security, dependency audit
   - *Output:* Security headers config, secrets checklist, secure API patterns
3. **Performance Engineer** — Hydration audit (every `client:*` justified), image optimisation, Core Web Vitals baseline, bundle analysis
   - *Output:* Lighthouse benchmark, optimised image pipeline, islands inventory
4. **Localization & i18n Engineer** — `[locale]` routing strategy, translation file structure, RTL considerations if needed
   - *Output:* i18n routing config, locale-aware content collections
5. **DevOps/IaC Specialist** — CI/CD pipeline (build → test → deploy), Vercel/adapter config, branch preview deployments, environment variables in CI
   - *Output:* Working pipeline, deploy config, preview URL per PR
6. **Technical Writer** — Content authoring guide for MDX/content collections, component usage docs for non-dev team members
   - *Output:* `CONTENT_GUIDE.md`, component storybook or README

**Success criteria:** Lighthouse 90+ on all pages, all routes pre-rendered (SSG), CI deploys on every PR, content team can author without touching code

---

### Astro Performance Audit

**Use for:** Existing Astro site with degraded Lighthouse scores or slow page load

**Chain:** Astro Specialist → Performance Engineer

**Workflow:**

1. **Astro Specialist** — Audit islands usage (over-hydrated components?), image usage (`<img>` vs `<Image>`?), build output analysis (`npx astro build --verbose`)
   - *Output:* Islands inventory, image audit, identified regressions
2. **Performance Engineer** — Core Web Vitals profiling, bundle size analysis, font loading, third-party script impact
   - *Output:* Prioritised fix list with effort/impact matrix

**Success criteria:** LCP < 2.5s, CLS < 0.1, INP < 200ms on all key pages

---

### Astro Feature Addition (SSR/API Route)

**Use for:** Adding dynamic behaviour (forms, auth, user-specific content) to an otherwise static Astro site

**Chain:** Astro Specialist → Security Sentinel → Backend/Distributed Systems Engineer

**Workflow:**

1. **Astro Specialist** — Switch route to `prerender = false` or migrate to hybrid mode, design API route structure, select adapter if not already set
   - *Output:* Updated `astro.config.mjs`, API route scaffolding
2. **Security Sentinel** — Input validation on API routes, CSRF protection for forms, rate limiting, auth token handling
   - *Output:* Secure API route patterns, validation middleware
3. **Backend/Distributed Systems Engineer** — Integration with Lambda backend (mdl-lambdas), error handling, response caching strategy
   - *Output:* Backend integration pattern, caching headers, error boundaries

**Success criteria:** Dynamic routes work in production, all inputs validated, no secrets exposed to client

---

## MDL Linear Issue Lifecycle

### Full Issue Lifecycle (Idea → Done)

**Chain:** Linear Workflow (create) → Linear Workflow (start) → Linear Workflow (progress) → Linear Workflow (PR) → Linear Workflow (status: Done)

**Condensed workflow for any DEV issue:**

```
1. CREATE   — New issue with MDL template (title, description, AC, technical notes)
2. ANALYSE  — Fetch issue, post task breakdown comment, estimate complexity
3. BRANCH   — feature/dev-{number}-{description}, empty init commit, push
4. BUILD    — Conventional commits referencing DEV-XXX throughout
5. UPDATE   — Progress comment at natural milestones (or end of day)
6. PR       — gh pr create with MDL PR template, post link to Linear
7. MERGE    — Status → Review Required, branch cleanup
8. DONE     — Status → Done after staging verification
```

**Trigger the whole chain with:** "Let's work on DEV-1234" or "Start DEV-1234"

---

### Quick Issue Triage

**Use when:** Multiple new issues need to be triaged at start of sprint or after discovery

**Chain:** Linear Workflow (get issues) → Pragmatic Architect (priority) → Linear Workflow (update status)

1. **Linear Workflow** — Fetch all Todo/Backlog issues for team DEV
2. **Pragmatic Architect** — Assess technical complexity, dependencies, risk
3. **Linear Workflow** — Update priorities and statuses, post estimation comments

**Output:** Sprint-ready issues with status = Ready for Development

---

### Incident → Linear Issue

**Use when:** Production incident needs a tracked follow-up

**Chain:** Incident Commander (lead) → Observability Engineer (diagnose) → Linear Workflow (create + start)

1. **Incident Commander** — Coordinate response, identify root cause
2. **Observability Engineer** — CloudWatch/Sentry analysis, determine fix scope
3. **Linear Workflow** — Create Bug issue with `priority: urgent`, start work immediately

**Branch format for hotfixes:** `fix/dev-{number}-{brief-description}`
**Commit prefix:** `fix(scope): [description] (DEV-XXX)` — flag as `priority: 1` in Linear


## MDL Standards Enforcement Workflows

### Pre-Commit Quality Chain
1. `security-standards` → Run pre-commit security checklist
2. `git-workflow` → Validate commit message format and branch naming
3. `changelog-standards` → Determine if CHANGELOG update needed

### New Feature Implementation Chain
1. `linear-workflow` → Create/start issue
2. `git-workflow` → Create branch with proper naming
3. *Domain skill* → Implement feature
4. `error-handling-standards` → Verify error handling patterns
5. `security-standards` → Security review
6. `performance-standards` → Performance check
7. `git-workflow` → Commit with conventional format
8. `linear-workflow` → Create PR and update status

### Production Release Chain
1. `changelog-standards` → Finalize CHANGELOG for version
2. `performance-standards` → Run Lighthouse audit, verify CWV targets
3. `security-standards` → Pre-deployment security checklist
4. `git-workflow` → Tag release
