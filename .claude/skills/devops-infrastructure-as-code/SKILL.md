...
# Install dependencies first if they've changed
...
- [ ] Verify function name in `package.json` matches intended environment (`dev` vs `prod`)
- [ ] Run `npm install` if `package.json` dependencies changed
- [ ] Test locally with mock event first (`node --experimental-vm-modules`)
- [ ] Check if shared module (`shared/`) was updated — redeploy all functions that import it
- [ ] Lambda runtime must be Node.js 18+ (ESM `.mjs` support required)

### Frontend Deployment

```bash
# Fan app (large bundle — needs extra memory)
...
```bash
npm run claude:rules       # compliance with .claude/*.md coding rules
npm run claude:security    # security audit of diff
npm run claude:patterns    # pattern compliance (service layer, React Query, etc.)
npm run claude:tests       # test coverage check vs TESTING_GUIDE.md
npm run claude:accessibility # WCAG 2.1 AA check
npm run claude:performance # bundle and render performance check
npm run test:ci            # Jest CI run
npm run test:e2e:ci        # Playwright CI run
npm run claude:pr          # generate PR description
```
...
```bash
# Branch naming (from Linear issue DEV-XXX)
...
- ❌ Deploying to `mdl-bck-prod-*` without testing in `mdl-bck-dev-*` first
- ❌ Forgetting `npm install` after adding a dependency to a function's `package.json`
- ❌ Deploying a function that imports from `shared/` without checking if `shared/` changed
- ❌ Running fan-dev build without `--max-old-space-size=8192` (OOM error)
- ❌ Deploying Lambda with CommonJS syntax (`.js` with `require()`) — MDL uses ESM only
