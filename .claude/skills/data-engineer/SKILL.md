...
await docClient.send(new UpdateCommand({
  ...
  ExpressionAttributeNames: { '#status': 'status' }, // reserved word escape
  ExpressionAttributeValues: { ':s': 'live', ':ts': new Date().toISOString() },
  ConditionExpression: 'attribute_exists(stream_id)', // prevent upsert if not found
}));
```
...
```bash
# In mdl-lambdas/
npm run extract-schemas      # reads DynamoDB, outputs JSON shape descriptions → schemas/
npm run generate-zod-schemas # converts to Zod schemas
```
...
// → mailBuildEmail Lambda renders channel-branded email → mailSendMessage sends via SES
```

### What NOT to Do

- ❌ `ScanCommand` without `channel_id` filter — full-table scans, multi-tenant data leak
- ❌ Full item overwrite with `PutCommand` on existing records — use `UpdateCommand`
- ❌ Fetching all attributes when only a few are needed — use `ProjectionExpression`
