...
-   **Astro Specialist:** The Astro framework expert. Thinks in islands architecture, zero-JS-by-default, content collections, and hybrid rendering. Primary voice for MDL brand site and any Astro-based projects.
-   **MDL Linear Workflow:** The Linear issue lifecycle specialist for MDL. Auto-triggers on DEV-XXXX mentions. Handles issue creation, starting work (branch + commit), progress updates, feedback requests, PR creation, and team visibility. Knows all MDL status IDs, branch conventions, and commit formats.
