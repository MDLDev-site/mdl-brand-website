...
## MDL Frontend Patterns

### Two Apps, Different Stacks

| | `mdl-fan-dev` | `mdl-admin-dev` |
|---|---|---|
| Language | TypeScript | JavaScript |
| Build | Vite + React 18 | Vite + React 18 |
| Components | MUI v5 + styled-components | Ant Design v5 + MUI charts |
| State (server) | TanStack Query v4 | TanStack Query v4 |
| Forms | react-hook-form + Zod v4 | react-hook-form |
| Video | Bitmovin Player + HLS.js | — |
| Auth | Cognito (`amazon-cognito-identity-js`) | Cognito |
| Payments | Stripe Elements | — |
| Error tracking | Sentry (`@sentry/react`) | — |
| Testing | Playwright + Jest + MSW | Jest |

### TanStack Query v4 — The State Management Pattern

TanStack Query handles **all server state**. No Redux, no Zustand, no `useEffect` for data fetching.

```typescript
// services/streams.ts — service layer, never fetch in components
export const fetchStreams = (channelId: string) =>
  axios.get<{ data: Stream[] }>(`/api/admin/streams`).then(r => r.data.data);

// hooks/useStreams.ts
export const useStreams = () =>
  useQuery(['streams'], fetchStreams, { staleTime: 30_000 });

// Mutation with cache invalidation
export const useCreateStream = () =>
  useMutation(createStream, {
    onSuccess: () => queryClient.invalidateQueries(['streams']),
  });
```

React Context is only for global UI state: auth session, theme, locale. Never for server data.
...
const config: PlayerConfig = {
  key: process.env.VITE_BITMOVIN_KEY,
  analytics: { key: process.env.VITE_BITMOVIN_ANALYTICS_KEY },
};

useEffect(() => {
  const player = new Player(containerRef.current, config);
  player.load({ hls: streamUrl, drm: drmConfig }).then(() => player.play());
  return () => player.destroy();
}, [streamUrl]);
```

Use Bitmovin as the primary player. HLS.js is a fallback for environments where Bitmovin can't load.
...
// Never store tokens manually — Cognito SDK manages session in memory/cookies
const userPool = new CognitoUserPool({ UserPoolId, ClientId });

// Auth state via React Context — not local state
const { user, isAuthenticated } = useAuth(); // custom hook wrapping Cognito
```

Never store Cognito JWTs in `localStorage`. Use the SDK's built-in session management.

### Form Pattern (fan-dev)
...
- ❌ Redux or Zustand for server state — TanStack Query owns it
- ❌ Direct `fetch`/`axios` calls in components — always go through service layer
- ❌ Storing auth tokens in `localStorage` — use Cognito SDK session
- ❌ Logging user PII (email, name) to Sentry — use user IDs only
- ❌ Adding TypeScript to `mdl-admin-dev` without team agreement — it's intentionally JS
